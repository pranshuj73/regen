...
export function buildResumePrompt(
				...
				(typeof updates === "object" &&
					((updates.combinedText && updates.combinedText.trim().length > 0) ||
						(updates.customUpdates &&
							updates.customUpdates.trim().length > 0) ||
	...
	if (hasUpdates) {
		if (typeof updates === "string") {
			...
			const selected =
				Array.isArray(updates.selectedIds) && updates.selectedIds.length
					? `Selected options: ${updates.selectedIds.join(", ")}`
					: "";
			const parts = [
				updates.combinedText,
				updates.customUpdates,
				selected,
	...
	if (hasUpdates)
		inputs.push(`- Desired updates from user (verbatim):\n${updatesText}`);
	if (hasJD)
		inputs.push(`- Target job description (verbatim):\n${jobDescription}`);
	sections.push(`INPUTS\n${inputs.join("\n")}`);

	const goals: string[] = [];
	if (hasJD) {
		goals.push(
			"1) Memorize and use the job description vocabulary to tailor content as well as tailor the resume to meet the requirements of the job description.",
		);
	} else {
		goals.push(
			"1) Optimize for common target roles using industry-relevant vocabulary.",
		);
	}
	if (hasUpdates) {
