...
export function UpdatesStep({
	...
	useEffect(() => {
		if (initialCustomUpdates !== undefined)
			setCustomUpdates(initialCustomUpdates);
	}, [initialCustomUpdates]);
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
		...
		const allUpdates = [...selectedDescriptions, customUpdates]
			.filter(Boolean)
			.join("\n");

		onSubmit(allUpdates);
		onSubmitWithMeta?.({
			selectedIds: selectedUpdates,
			customUpdates,
			combinedText: allUpdates,
		});
	};
		...
		<Card className="w-full h-full">
			...
			<CardContent className="space-y-6">
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<div>
						<Label htmlFor="custom-updates" className="mb-2">
						...
						<Textarea
							...
							onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
								setCustomUpdates(e.target.value)
							}
							rows={4}
