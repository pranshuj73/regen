...
export function ResumeComponent({ data }: ResumeComponentProps) {
	return (
		<div
			className="max-w-4xl mx-auto p-4 font-sans text-black bg-white resume-print"
			style={{
				marginTop: "1rem",
				marginBottom: "1rem",
			}}
		>
			...
			{data.summary && (
				<div className="mb-4">
					<h2 className="text-sm font-bold border-b border-black pb-1 mb-2 uppercase">
						SUMMARY
					</h2>
				...
				<div className="mb-4">
					...
					{data.technical_skills.development_tools.length > 0 && (
						<p className="text-xs mb-1">
							...
							{data.technical_skills.development_tools.join(", ")}
						</p>
					)}
					{data.technical_skills.libraries.length > 0 && (
						<p className="text-xs mb-1">
					...
					<div key={index} className="mb-3">
						...
						<ul className="mt-1 ml-4 space-y-0.5">
							{exp.responsibilities.map((resp, respIndex) => (
								<li key={respIndex} className="text-xs leading-relaxed">
									• {resp}
								</li>
							))}
						</ul>
